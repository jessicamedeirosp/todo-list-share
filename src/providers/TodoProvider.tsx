...
export function TodoProvider({ children }: TodoProviderProps) {
  ...
  function readTodo(id: number) {
    try {
      ...
      if (newTodo) setTodoCurrent(newTodo);
      return newTodo;
    } catch (e: any) {
      toast.error(e);
    }
  ...
  function handleMoveItem({
    ...
    const items = isSubitem
      ? findItem(parent, todoCurrent.items)?.items
      : todoCurrent.items;
    const todo =
      isSubitem && items ? findItem(parent, todoCurrent.items) : todoCurrent;
    const target =
      isSubitem && items && todo
        ? findSubitem(id, todo)
        : id && items && findItem(id, items);
